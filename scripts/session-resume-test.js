...
check('Einladungslink rendert minimalistischen Join-Flow mit Absender und Zurueck-Link', /if \(joinLinkCode\) \{[\s\S]*Einladung von \$\{inviter\}[\s\S]*type="hidden" id="codeInput" value="\$\{joinLinkCode\}"[\s\S]*Mitspielen[\s\S]*return;/.test(html) && !/Einladung für Raum \$\{joinLinkCode\}/.test(html) && !/Beitreten!<\/button>[\s\S]*<\/div>\s*<\/div>\s*`\;\s*return;/.test(html));
check('Link teilen ist wieder aktiv, mit Absender, aber ohne Auto-Resume-Magie', /onclick="shareRoom\(\)"/.test(html) && /async function shareRoom\(\)/.test(html) && /navigator\.share/.test(html) && /params\.set\('from', inviter/.test(html) && /joinLinkFrom =/.test(html) && !/markResumeIntent/.test(html) && !/mk-resume-intent/.test(html));
